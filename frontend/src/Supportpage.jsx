...
export default function SupportPage() {
  ...
  /* Fetch all users, sender profile, and received requests */
  useEffect(() => {
    if (!user) return;
    Promise.all([
      getDocs(collection(db, "users")),
      getDocs(query(collection(db, "helpRequests"), where("toUserId", "==", user.uid))),
    ]).then(([usersSnap, receivedSnap]) => {
      ...
      setReceivedRequests(receivedSnap.docs.map((d) => ({ id: d.id, ...d.data() })));
    });
  }, [user]);
  ...
  const handleRespondRequest = async (reqId, status) => {
    try {
      const responderName = senderProfile ? getFullName(senderProfile) : user.email;
    ...
    <div style={S.page}>
      ...
      {receivedRequests.length > 0 && (
        <div style={S.receivedReqSection}>
          <p style={S.myReqTitle}>Received Requests</p>
          <div style={S.myReqGrid}>
