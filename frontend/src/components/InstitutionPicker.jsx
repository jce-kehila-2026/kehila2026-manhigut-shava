...
export default function InstitutionPicker({ value, translations, onChange, placeholder }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      let id;
      try {
        const docRef = await addDoc(collection(db, "customInstitutions"), { ...t, addedAt: serverTimestamp() });
        id = docRef.id;
      } catch {}
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteDoc(doc(db, "customInstitutions", id));
      cachedCustomInstitutions = (cachedCustomInstitutions || []).filter(c => c.id !== id);
