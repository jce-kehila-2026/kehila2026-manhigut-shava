...
import { REGIONS_BY_LANG, translateRegion } from "../utils/translateProfile";
...
let cachedCustomRegions = null;

export default function RegionPicker({ value, translations, onChange, placeholder }) {
  ...
  const [custom, setCustom] = useState(cachedCustomRegions || []);
  const [adding, setAdding] = useState(false);
  const containerRef        = useRef();
  const inputRef            = useRef();

  useEffect(() => {
    if (cachedCustomRegions) return;
    getDocs(collection(db, "customRegions")).then(snap => {
      const list = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      cachedCustomRegions = list;
      setCustom(list);
    }).catch(() => {});
  }, []);
  ...
  const knownOptions = (REGIONS_BY_LANG[lang] || REGIONS_BY_LANG.he)
    .map((label, i) => ({
      label,
      enValue: REGIONS_BY_LANG.en[i],
      translations: {
        he: REGIONS_BY_LANG.he[i],
        en: REGIONS_BY_LANG.en[i],
        ar: REGIONS_BY_LANG.ar[i],
      },
    }));
  ...
  const displayValue = translations?.[lang] || translateRegion(value, lang) || value || "";
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      let id;
      try {
        const docRef = await addDoc(collection(db, "customRegions"), { ...t, addedAt: serverTimestamp() });
        id = docRef.id;
      } catch {}

      const entry = { id, he: t.he, en: t.en, ar: t.ar };
      cachedCustomRegions = [...(cachedCustomRegions || []), entry];
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteDoc(doc(db, "customRegions", id));
      cachedCustomRegions = (cachedCustomRegions || []).filter(c => c.id !== id);
