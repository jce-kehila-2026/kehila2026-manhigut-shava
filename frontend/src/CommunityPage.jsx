...
/* ── Comment item ── */
function CommentItem({ comment, currentUid, isAdmin, onDelete }) {
  return (
    <div style={{ display: "flex", gap: 8, paddingTop: 10 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{
          ...
          <p style={{ fontSize: 13, color: "var(--text-secondary)", marginTop: 2, lineHeight: 1.5 }}>{comment.text}</p>
        </div>
...
function PostCard({ post, currentUser, isAdmin, onDelete }) {
  ...
  return (
    <article className="card slide-up" style={{ marginBottom: "1rem" }}>
      ...
      {/* Comments section */}
      {showComments && (
        <div style={{ padding: "0.75rem 1.25rem 1rem", borderTop: "1px solid var(--bg-tertiary)", animation: "slideDown 0.2s ease" }}>
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, "posts"), {
        ...
        media: mediaUrls,
        authorId: currentUser.uid,
        authorName: profile ? `${profile.firstName} ${profile.lastName}` : currentUser.email,
